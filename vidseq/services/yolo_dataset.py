...
def collect_training_samples(
    ...
    Uses SQLite query to get all training frames with bboxes efficiently.
    ...
    with Session(project_engine) as session:
        for video in videos:
            ...
            # Get all training frames with bboxes in one efficient query
            training_data = frame_data_service.get_training_frames_with_bboxes_sync(
                session, video.id
            )

            for frame_idx, bbox in training_data:
                # Validate bbox
                x1, y1, x2, y2 = bbox
                if x1 >= x2 or y1 >= y2:
                    continue
