"""Keypoint tracking worker port and PID management utilities."""
...
PORT_FILE = APP_DATA_DIR / "keypoint_tracking.port"
PID_FILE = APP_DATA_DIR / "keypoint_tracking.pid"
...
def get_keypoint_tracking_port() -> Optional[int]:
    """
    Read keypoint tracking worker port from port file.
...
def write_keypoint_tracking_port_file(port: int) -> None:
    """Write port number to port file."""
    APP_DATA_DIR.mkdir(parents=True, exist_ok=True)
    PORT_FILE.write_text(str(port))


def write_keypoint_tracking_pid_file(pid: int) -> None:
    """Write process ID to PID file."""
    APP_DATA_DIR.mkdir(parents=True, exist_ok=True)
    PID_FILE.write_text(str(pid))


def is_keypoint_tracking_worker_running() -> bool:
    """
    Check if keypoint tracking worker is running by checking port file and PID.
...
def cleanup_keypoint_tracking_port_files() -> None:
